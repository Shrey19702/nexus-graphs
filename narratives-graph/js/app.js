...
import {
  ...
  truncateLabel,
  updateZoomDisplay as sharedUpdateZoomDisplay,
  pointerToGraph as sharedPointerToGraph,
...
function updateZoomDisplay() {
  sharedUpdateZoomDisplay(zoomLabel, state);
}

function pointerToGraph(event) {
  return sharedPointerToGraph(event, canvas, state);
}

function panelBleedWidth() {
  return sharedPanelBleedWidth(panelEl, PANEL_MAX_W);
}

function settingsBleedWidth() {
  return sharedSettingsBleedWidth(settingsEl);
}

function drawNodeCircle(n, fill, stroke, strokeWidth) {
  sharedDrawNodeCircle(ctx, n, fill, stroke, strokeWidth);
}

function appendField(parent, label, valueEl) {
  return sharedAppendField(parent, label, valueEl);
}

function makeValue(text, mono = false) {
  return sharedMakeValue(text, mono);
}

function makeLink(href, label) {
  return sharedMakeLink(href, label);
}
